...
test("persist keepAlive process dies when jarmuz exits on SIGTERM", async function (t) {
  const consumerProject = await copyConsumerProject("persist-keepalive");
  ...
  const { child, closed } = runNodeScript(
    ...
    ),
    {
      ...
      env: {
        ...
        JARMUZ_PIPELINE: JSON.stringify(["server"]),
